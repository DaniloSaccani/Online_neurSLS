...
class gain(torch.nn.Module):
    def __init__(self,gamma_bar):
        super().__init__()
        self.gamma = torch.nn.Parameter(torch.ones(1)*torch.sqrt(gamma_bar))**2
    def forward(self, u):
        usys = self.gamma  * u
        return usys
...
class RenG(nn.Module):
    ...
    def __init__(self, m, p, n, l, bias=False, mode="l2stable", gamma=1, Q=None, R=None, S=None, x0=None,
        ...
        if x0 is not None:
            ...
            self.x0 = nn.Parameter((torch.zeros(1, n, device=device) * std))
        ...
        self.D12 = nn.Parameter(torch.randn(l, m, device=device) * std)
